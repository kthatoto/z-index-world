...
let isRunning = false;
let rafId = null;
let colliderUpdateInterval = null;
// ============================================================================
// Initialization
// ============================================================================
function initGame() {
...
function scanAndBuildColliders() {
    ...
    for (const el of allElements) {
        ...
        if (rect.width === 0 || rect.height === 0)
            continue;
        // Skip tiny elements
        if (rect.width * rect.height < MIN_ELEMENT_AREA)
            continue;
        // Parse z-index
...
function normalizeZValues(rawZIndexes) {
...
function renderDebugWalls() {
    if (!debugWallsContainer)
        return;
    // Clear existing
    debugWallsContainer.innerHTML = '';
    // Sort by area (descending) and take top N
    const sorted = [...colliders].sort((a, b) => (b.aabb.w * b.aabb.h) - (a.aabb.w * a.aabb.h));
