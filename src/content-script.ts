...
interface Collider extends Box {
  element: Element;
}
...
let allColliders: Collider[] = [];
let activeColliders: Collider[] = [];
let grid: Map<string, Collider[]> = new Map();
...
function resolveCollision(pBox: Box, col: Collider): { axis: 'x' | 'y' | 'z', delta: number } | null {
  if (!intersects(pBox, col)) return null;
...
function getColliderCells(col: Collider): string[] {
  const keys: string[] = [];
...
function getNearbyColliders(pBox: Box): Collider[] {
  ...
  for (let cx = x1; cx <= x2; cx++) {
    for (let cy = y1; cy <= y2; cy++) {
      ...
      if (cols) {
        for (const c of cols) result.add(c);
      }
    }
  }
...
function scanColliders() {
  ...
  const candidates: { el: Element; rect: DOMRect; zIndex: number }[] = [];
...
function pickStartGoal() {
  ...
  const visible = activeColliders.filter(c =>
    c.x >= -50 && c.x + c.w <= vw + 50 &&
  ...
  for (const c of candidates) {
    ...
    const cy = c.y + c.h / 2;
    let penalty = 0;
    if (cx < 0 || cx > vw || cy < 0 || cy > vh) penalty = -1000;
...
function updateMarkerTransform(el: HTMLElement, col: Collider) {
...
function updateMarkers() {
  if (startMarkerEl && startCollider) updateMarkerTransform(startMarkerEl, startCollider);
  if (goalMarkerEl && goalCollider) updateMarkerTransform(goalMarkerEl, goalCollider);
}
...
function createTestCube(x: number, y: number, z: number, size: number, color: string, label: string) {
...
function renderDebugColliders() {
  if (!debugContainer || !DEBUG) return;
...
function resolveAxis(axis: 'x' | 'y', nearby: Collider[]) {
  const pBox = getPlayerBox();
  for (const col of nearby) {
    if (intersects(pBox, col)) {
      const resolution = resolveCollision(pBox, col);
      if (resolution && resolution.axis === axis) {
        if (axis === 'x') player.x += resolution.delta;
        if (axis === 'y') player.y += resolution.delta;
      }
    }
  }
}
