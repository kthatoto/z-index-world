...
let startBox: Collider | null = null;
let goalBox: Collider | null = null;
...
function getCandidates(box: Box): Collider[] {
  ...
  for (let cx = x1; cx <= x2; cx++) {
    for (let cy = y1; cy <= y2; cy++) {
      ...
      if (cols) for (const c of cols) result.add(c);
    }
  }
...
function updateMarkerTransform(el: HTMLElement, box: Collider) {
...
function resolveX(candidates: Collider[]) {
  for (const col of candidates) {
    if (!intersects(player, col)) continue;
...
function resolveY(candidates: Collider[]) {
  for (const col of candidates) {
    if (!intersects(player, col)) continue;
...
function resolveZ(candidates: Collider[]) {
  isGrounded = false;
...
function onScrollResize() {
  updateColliderRects();
  renderDebug();
  if (startMarkerEl && startBox) updateMarkerTransform(startMarkerEl, startBox);
  if (goalMarkerEl && goalBox) updateMarkerTransform(goalMarkerEl, goalBox);
}
